...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;
    // Check for active events where this worker is assigned
    const { data: allEvents } = await db().from('events').select('id, data');
    const activeEvents = (allEvents || []).filter(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (e: any) =>
            !['Cerrado', 'Cancelado'].includes(e.data?.estado) &&
            Array.isArray(e.data?.trabajadoresAsignados) &&
            e.data.trabajadoresAsignados.includes(id)
    );
    if (activeEvents.length > 0) {
        return NextResponse.json(
            { error: `No se puede eliminar: el trabajador está asignado a ${activeEvents.length} evento(s) activo(s).` },
            { status: 409 }
        );
    }
